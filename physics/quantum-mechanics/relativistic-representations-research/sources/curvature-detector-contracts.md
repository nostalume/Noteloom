...
Consumed by: [carrier, source, and observable compiler](../nodes/15-carrier-source-obstructions.md).

Status: primary-source boundary for node 15; its departure maps, spectral
...
- **Input consumed:** the positive one-particle map already constructed in node 05.
- **Output consumed:** in the corresponding quasifree representation, a smeared
  ...
  creation part.
- **Boundary:** the source treats the scalar theorem directly. node 15 uses the
...
- **Output consumed:** the generalized curvature is an order-`s` differential
  ...
  annihilator; node 15 consumes only its derivative degree and physical shell action.
...
- **Research use:** this delimits the model class. node 15 does not import the
...
- **Boundary:** a scalar derivative detector is not evidence for a universal
  matter coupling to every higher-spin curvature. node 15's detector remains an
...
- **Output consumed:** the covariant coherent-state family admits a resolution of
  the identity.
- **Research use:** node 15 does not import its normalization. It constructs the
...
- **Research use:** node 15 reuses node 11's internally derived finite-time kernel for its
