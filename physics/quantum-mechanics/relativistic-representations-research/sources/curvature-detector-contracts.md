...
Status: primary-source boundary for N10u/N10v; their departure maps, spectral
...
- **Input consumed:** the positive one-particle map already constructed in N4g/N4y.
- **Output consumed:** in the corresponding quasifree representation, a smeared
  ...
  creation part.
- **Boundary:** the source treats the scalar theorem directly. N10u uses the
...
- **Output consumed:** the generalized curvature is an order-`s` differential
  operation. N10j independently constructs the chiral operation and its gauge
  annihilator; N10u consumes only its derivative degree and physical shell action.
...
- **Research use:** this delimits the model class. N10u does not import the
...
- **Boundary:** a scalar derivative detector is not evidence for a universal
  matter coupling to every higher-spin curvature. N10u's detector remains an
...
- **Output consumed:** the covariant coherent-state family admits a resolution of
  the identity.
- **Research use:** N10u does not import its normalization. It constructs the
...
- **Research use:** N10u reuses N9d's internally derived finite-time kernel for its
