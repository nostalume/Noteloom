...
finite-spin polynomial complexes. node 05 construct their shell maps, quotient
...
- **Internal Minkowski computation in node 05:** the retarded and advanced scalar
...
- **Boundary:** the exact scalar coefficient depends on the Fourier convention and
  ...
  reverse. node 05 retains one real nonzero normalization constant instead of hiding
  this choice.
...
- **Research use:** independent evidence that the route
  ...
  is the correct analytic kind of bridge. node 05 does not import Lledo's spinor
  formulas or use them as a proof for the symmetric-potential source quotient.
  node 05 uses the paper's Weyl and arbitrary odd-rank fermionic cases as an external
...
- **Boundary:** continuity and density must be checked for the particular source
  ...
  prove density of node 05's compact, projected-conserved symmetric sources.
...
[local complex and free-field machine](../nodes/05-local-symbol-complex.md). The complete finite-type
...
2. show that an `L2` shell section annihilating every admissible compact source
   defines the zero distributional solution, then use node 05's source/solution
   duality.

node 05 proves that pointwise screen exactness alone was insufficient for (a), then
supplies the missing support-preserving lift. Consequently node 05's norm is faithful
...
- **Internal construction in node 05:** the positive spinor metric is derived from the
...
- **Boundary:** the source does not prove that every shellwise gauge amplitude for
  node 05's symmetric spinor-tensor potential has one spacelike-compact gauge lift.
