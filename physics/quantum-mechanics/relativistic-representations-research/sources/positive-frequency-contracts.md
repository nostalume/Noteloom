...
Recorded: 2026-08-29  
Used by: [N4g integer-spin positive-frequency completion](../nodes/04g-positive-frequency-completion.md) and [N4k half-integer positive-energy completion](../nodes/04k-half-integer-positive-frequency.md)

This packet binds only the analytic statements that are not consequences of the
finite-spin polynomial complexes. N4g and N4k construct their shell maps, quotient
...
- **Internal Minkowski computation in N4g:** the retarded and advanced scalar
...
- **Boundary:** the exact scalar coefficient depends on the Fourier convention and
  ...
  reverse. N4g retains one real nonzero normalization constant instead of hiding
  this choice.
...
- **Research use:** independent evidence that the route
  ...
  is the correct analytic kind of bridge. N4g does not import Lledo's spinor
  formulas or use them as a proof for the symmetric-potential source quotient.
  N4k uses the paper's Weyl and arbitrary odd-rank fermionic cases as an external
...
- **Boundary:** continuity and density must be checked for the particular source
  ...
  prove density of N4g's compact, projected-conserved symmetric sources.
...
[N4h](../nodes/04h-support-faithfulness.md). The complete finite-type
...
2. show that an `L2` shell section annihilating every admissible compact source
   defines the zero distributional solution, then use N4f's source/solution
   duality.

N4h proves that pointwise screen exactness alone was insufficient for (a), then
supplies the missing support-preserving lift. Consequently N4g's norm is faithful
...
- **Internal construction in N4k:** the positive spinor metric is derived from the
...
- **Boundary:** the source does not prove that every shellwise gauge amplitude for
  N4k's symmetric spinor-tensor potential has one spacelike-compact gauge lift.
