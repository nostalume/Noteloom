...
[equivalence and dynamical reduction](../nodes/08-realization-equivalence-boundary.md). They distinguish a
controlled deformation of the node 08 breather from the exact factorization that
made node 08 solvable.
...
- **Output consumed:** to first order,
  ...
  kinematics.
- **Research use:** node 08 turns the exact node 08 rest mass into a one-form-factor
...
- **Research use:** at node 08's benchmark `xi=1/5`, `beta^2=4 pi/3` and
...
- **Boundary:** node 08 has reduced the nonintegrable tangent to these values but has
...
- **Output consumed:** a systematic finite-volume formulation of higher-order
  ...
  calculations.
- **Research use:** node 08 places second and higher orders in a computation node;
...
- **Boundary:** the published double-sine-Gordon evaluation uses its own
  ...
  missing numerical coefficient for node 08's second-harmonic benchmark.
...
- **Research use:** this confirms that failure of node 08's single-soliton fusion
