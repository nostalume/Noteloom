...
This packet binds only the theorem-scale statements that node 08 does not reproduce.
...
- **Output consumed:** projection to occupation pointer states; the second-order
  ...
  SSEP.
- **Internal use:** node 08 evaluates this operator on the same occupation projector
  and obtains its rate in node 08's `J,gamma` convention.
...
- **Internal use:** this contract supports promoting node 08's exact current
...
- **Boundary:** the paper uses its own hopping and dephasing normalization and an
  imaginary-coupling Hubbard/Bethe representation. node 08 does not import those
...
- **Internal use:** node 08 uses this as a scope check for its observable-relative
...
function agrees with classical SSEP. node 08 records this as a downstream candidate,
...
- **Internal use:** node 08 uses this as evidence that finite dephasing need not destroy
  SSEP hydrodynamic current statistics at large scale.
- **Boundary:** this is not node 08's infinite-line annealed equilibrium current and
  not node 08's isolated-system two-projective-measurement charge difference. The
