...
[equivalence and dynamical reduction](../nodes/08-realization-equivalence-boundary.md). They separate exact
...
- **Output consumed:** an exactly solvable `1+1`-dimensional gauge model can
  ...
  mass term.
- **Research use:** node 08 retains the massless Schwinger model as a semantic-
...
- **Output consumed:** soliton and antisoliton asymptotic particles of mass `M_s`;
  ...
  of `B_1`.
- **Research use:** node 08 reconstructs the mass formula from the pole rapidity and
...
- **Research use:** node 08 interprets a breather pole as a fermion--antifermion bound
...
- **Research use:** node 08 selects an odd local combination with nonzero `B_1`
  one-particle form factor, which supplies node 08's nonzero field-access map.
...
- **Research use:** this contract can replace node 08's physical input `M_s` when the
  ultraviolet normalization is fixed exactly. node 08 keeps `M_s` as its primary
...
- **Research use:** node 08 uses this as evidence that fusion-pole data participate in
...
- **Boundary:** self-adjointness, strong commutativity, nontrivial local
  ...
  open in the cited construction. node 08 therefore does not label its bootstrap
