...
### G16 — Complete-route leverage and human-cost adjudication (`mixed bounded`)
...
- **Falsifier:** the comparison omits G8/G14 construction, changes the observable
  ...
  to a family claim.
- **Result:** [the complete-route leverage comparison](complete-route-leverage.md)
  admits a fixed G10 model, local coefficient jet, finite window, preparation,
  complement observable, and accuracy. G14--G15 is compared with a three-sample
  Hermitian-eigensolver projector baseline. Multiplicities two and three and
  windows one and four agree within `3.88e-8`. Exact work removes sampling,
  eigensolves, and two declared numerical choices, but retains two Sylvester
  blocks and the same dense propagation.
- **Disposition:** exact-route runtime dominance is refuted on all four observed
  cases: the sampled route is `38.3--299.6` times faster in this environment.
  Exact construction has lower declared semantic depth (`4` objects and `6`
  transformations versus `6` and `8`) but greater theorem debt. Heterogeneous
  operation proxies admit no scalar dominance without weights. Original-PDE
  propagation remains explicitly unpaid, so this is not an end-to-end PDE verdict.
...
### G17 — Prepared-observable active-subspace propagation (`mixed bounded`)

- **Upstream:** G15 constructs low-rank off-block arrows, while G16 shows that both
  compared routes still pay for dense propagation on the full ambient carrier.
- **Tension:** representation structure removes coordinate choices but its
  computational benefit is relocated unless directions unreachable from the
  supplied preparation and observable are removed before exponentiation.
- **Question:** can the exact Krylov closure generated by each `H(k)` and prepared
  carrier state construct the minimal active invariant subspace, propagate there,
  recover the same complement probability, and lower complete route cost?
- **Alternatives:** observable-cyclic reduction before propagation versus dense
  full-carrier exponentiation or independent leakage-column bounds.
- **Falsifier:** the reduced subspace is not invariant, compression changes the
  complement effect, construction costs erase the solve saving, Gaussian-complex
  Hermitian data are forced through the current real-rational cyclic API, or the
  tested family has no dimension gain.
- **Result:** [the prepared-observable active constructor](prepared-observable-active-subspace.md)
  grows the exact Gaussian-rational Krylov carrier until its first dependence,
  constructs `H B=B A`, retains self-adjointness as `G A=A^dagger G`, and pulls
  the effect back as `F=B^dagger Q B`. It does not require `Q` to preserve the
  carrier and does not coerce complex data through the real cyclic API. G14--G17
  can now run without first evaluating the ambient exponential.
- **Disposition:** all G16 transfer blocks reduce to dimension three from ambient
  dimensions four or six and recover the same window observable within
  `1.39e-17`. One-shot runtime dominance is refuted: active construction is
  `9.01--59.38` times slower on these small blocks. Reusing the exact carrier for
  64 times reverses the measured order in all four cases, with active/dense ratios
  `0.25--0.68`. Thus finite exact and amortized leverage are supported, not
  universal or one-shot speed.
- **Boundary:** finite exact carrier, supplied preparation/effect, and finite time.
  Continuous spectral measures, unbounded domains, and multi-coordinate curvature
  remain separate.

## Active open bridge

### G18 — Coefficient-family reachable module

- **Upstream:** G17 constructs a minimal carrier separately for every assembled
  `H(k)`, although the observed active dimension is constant across the momentum
  window and the exact construction dominates one-shot cost.
- **Tension:** pointwise Krylov reduction detects hidden representation
  multiplicity but repeats the same discovery at every parameter value.
- **Question:** given `H(k)=sum_a f_a(k) H_a`, can closure of the preparation under
  the represented coefficient algebra generated by the `H_a` construct one
  finite module that every specialization preserves, while retaining the same
  pulled-back effect and reducing whole-window construction cost?
- **Alternatives:** one coefficient-family cyclic module specialized across the
  window versus independent pointwise carriers; a sufficient common module may
  also expand to the full ambient representation and destroy the gain.
- **Falsifier:** coefficient closure depends on sampled parameter values, fails
  specialization invariance, changes the observable, grows to the ambient carrier
  on the G16 transfer family, or costs no less than the repeated pointwise route.
- **Next probe:** extract the constant and momentum-linear Hamiltonian
  coefficients from the G15 family, close the prepared state under both actions
  with exact bounded breadth-first generation, and compare common dimension,
  specialization recovery, and window/time amortization with G17.
- **Boundary:** finite exact coefficient families with fixed preparation and
  effect. Variable domains, non-polynomial operator families, continuous direct
  integrals, and nonlinear closure remain separate.
