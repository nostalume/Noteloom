...
| `finite_window_propagation.py` | exact finite Hamiltonian assembly separated from dense propagation, preparation-specific bounds, and residual-bearing recovery |
...
| `gaussian_active_subspace.py` | minimal prepared Krylov carrier over `Q(i)`, Gram self-adjointness, effect pullback, and reusable-time evaluation |
| `active_window_propagation.py` | standalone G14--G17 active route plus same-window dense-reference adjudication |
| `complete_route_bench.py` | shared repeated-Pauli domain adapter for G16/G17 transfer evidence |
| `benchmark_active_subspace.py` | reproducible one-shot and multi-time dense/active solve-stage comparison |
...
G17 separates exact Hamiltonian assembly from propagation, constructs the minimal
Gaussian-rational module generated by each preparation, and compresses both
Hamiltonian and effect. Run `uv run python benchmark_active_subspace.py` to replay
the solve-stage comparison. The current family supports exact recovery and
64-time amortization, while refuting one-shot runtime dominance.
